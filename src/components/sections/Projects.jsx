  ...
  return (
    <motion.div 
      ...
      whileHover={{ 
        y: -10,
        scale: 1.02,
        borderColor: 'rgba(197, 208, 152, 0.4)',
        boxShadow: '0 30px 60px -15px rgba(0, 0, 0, 0.4)'
      }}
      transition={{ type: 'spring', stiffness: 200, damping: 20 }}
      className="group overflow-hidden rounded-3xl bg-gray-900 border border-white/10 shadow-sm"
    >
      <div className="p-8">
        {/* Header */}
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {project.tech.map((tech) => (
            <motion.span
              key={tech}
              whileHover={{ scale: 1.05, backgroundColor: 'rgba(197, 208, 152, 0.1)', color: '#C5D098' }}
              className="px-2.5 py-1 text-xs font-medium bg-gray-800 text-gray-300 rounded cursor-default border border-transparent hover:border-[#C5D098]/20 transition-all duration-200"
            >
              {tech}
            </motion.span>
          ))}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {project.stars && (
              <div className="flex items-center gap-1 text-xs text-gray-500">
                <Star className="h-3 w-3 fill-current text-[#C5D098]" />
          ...
          <motion.a
            ...
            className="text-sm font-medium text-[#C5D098] hover:text-[#B5C088] transition-colors flex items-center gap-1"
            whileHover={{ x: 3 }}
          >
  ...
  return (
    <section id="projects" className="py-32 overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Call To Action Button (Scroll Reveal) */}
        <motion.div 
        ...
        >
          <motion.a
            href="/projects"
            className="inline-flex items-center px-6 py-3 border border-gray-600 text-gray-300 hover:border-[#C5D098] hover:text-white transition-colors rounded-full font-medium"
            whileHover={{ scale: 1.05, y: -2, borderColor: 'var(--text-color)' }}
            whileTap={{ scale: 0.95 }}
          >
            View All Projects
